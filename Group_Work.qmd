...
# Composition of airbnb listings with respect ghost hotel

number1Df = pd.DataFrame(df["neighbourhood_cleansed"].value_counts())
number2Df = pd.DataFrame(ghostHotelDf_unique["neighbourhood_cleansed"].value_counts())
numberDf = pd.merge(number1Df, number2Df, how='outer',left_index=True,right_index=True)
numberDf.columns = ["total listings", "ghost hotels"] 
...
def stackFigure():
    ...
    sns.barplot(x = numberDf.index, y = numberDf["total listings"], color = "#bae4bc")
    ghost_plot = sns.barplot(x = numberDf.index, y = numberDf["ghost hotels"], color = "#43a2ca")
    raw_data_legend = plt.Rectangle((0,0),1,1,fc="#bae4bc", edgecolor = 'none')
    ghost_hotel_legend = plt.Rectangle((0,0),1,1,fc='#43a2ca',  edgecolor = 'none')
...
The @Insideairbnb data was collected by a team led by Murray Cox.
Collaborators: Murray Cox, John Morris, Taylor Higgins
Past collaborators: Alice Corona, Luca Lamonaca, Michael "Ziggy" Mintz, Anya Sophe Behn
Independent advisory board: Alice Corona, Claire Colomb, Peter Phibbs, Sito Veracruz, Thorben Wieditz
...
The data sets of @Insideairbnb were assembled by web scraping techniques from  Airbnb’s website, but they were implemented and collected independently. 

Both data sets attempt to locate all the listings within a city, and then visit the page for each listing to collect listing data, including the host ID. The host ID allows an analysis of the number of listings posted by a single host. 
...
The data collection methodology of @Insideairbnb, while innovative, has important implications for the completeness and accuracy of their dataset. The web scraping approach effectively captures public listings but inherently misses private or temporarily hidden properties. Since data collection occurs at monthly intervals, rapid market changes might go unrecorded, creating potential blind spots in understanding market dynamics.

The project's use of the "San Francisco Model" for estimating occupancy and income introduces additional complexity. While this model provides structured estimates, it may not accurately reflect the nuanced variations across different markets or capture actual booking patterns. The inability to access real transaction data or direct communications between hosts and guests further limits our understanding of platform usage patterns.

These methodological constraints raise significant issues for policy-making. Cities and regulators, working with estimates and incomplete information, face challenges in crafting effective regulations. This becomes particularly problematic when trying to distinguish between professional operators and casual hosts, or when assessing the true impact of short-term rentals on housing availability.

The methodology also presents interesting questions about transparency in the digital age. While @Insideairbnb aims to increase market transparency, their data collection limitations create their own form of opacity. Researchers and policymakers must carefully balance the value of available data against its known limitations, often needing to seek additional data sources for a more complete understanding of market dynamics.
...
How to identify ghost hotels from @Insideairbnb data is key to analysing this phenomenon. Since studies on ghost hotels are limited, we combined observed characteristics and previous studies to determine the method for identification. First, we found the hosts with multiple listings, calculated the geographic distance for all of their listings and used NLP to measure the similarity of neighborhood overviews and descriptions. If the distance is less than 200 meters and the text description similarity is greater than 0.5, it will be considered as ghost hotel. We finally identified 43,584 potential ghost hotels.

```{python}
stackFigure()
```

Figure 1 shows the proportion of ghost hotels in Airbnb listings in the 33 boroughs of London. It can be seen that the total number of listings in Westminster is significantly greater than other boroughs, followed by Tower Hamlets, Camden and others. The number of ghost hotels in these boroughs is also noticeably higher.

```{python}
ghProportionPlot()
```

Figure 2 shows the distribution of the proportion of ghost hotels across London boroughs. It can be seen that the areas with high proportions of ghost hotels are concentrated in central London, with the City of London having the highest proportion. The outer boroughs have significantly lower levels of ghost hotels.
...
- Li H, Kim Y, Srinivasan K. Market shifts in the sharing economy: The impact of Airbnb on housing rentals[J]. Management Science, 2022, 68(11): 8015-8044.
